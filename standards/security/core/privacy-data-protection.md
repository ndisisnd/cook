...
## NEVER
- Use weak/outdated cryptographic primitives for data at rest or in transit (see `crypto-algorithms`)
...
- Hash passwords with Argon2id (or scrypt/bcrypt as documented in `authentication-mfa`) using a per-user salt
...
- Provide privacy-focused audit trails and access logging (see `logging`)
