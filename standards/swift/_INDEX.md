...
| swift → interop ref | — | CoreFoundation, Unmanaged, takeRetainedValue, UnsafePointer, bridging header, @objc, KVO |

> Load `<SKILLS>/swift/SKILL.md` for any `.swift` file — it covers language correctness (P0) and style conventions (P1).
> Pull a ref only when the task needs that depth. Platform/app concerns (scenes, sandboxing, distribution, persistence, localization) live in `<SKILLS>/macos/`, not here.
